component {
	...
	function startList(any rc) {
		...
		rc.data = getQuestionService().list();
	}
	...
	function startPost(any rc) {
		...
		if(arrayLen(rc.errors)) {
			...
			rc.data = getQuestionService().post(trim(rc.title),trim(rc.text),rc.user);
		}
	}
	...
	function startPostAnswer(any rc) {
		try{
			...
			getQuestionService().postAnswer(rc.question,rc.answer,rc.user);
		}
		catch(any e){
			writeDump(e);
			abort;
	...
	function startSelectAnswer(any rc) {
		...
		getQuestionService().selectAnswer(rc.question,rc.answerid);
	}
	...
	function startVoteAnswerDown(any rc) {
		loadQuestion(rc);
		getQuestionService().voteAnswerDown(rc.question,rc.answerid,rc.user);
	}
	...
	function startVoteAnswerUp(any rc) {
		loadQuestion(rc);
		getQuestionService().voteAnswerUp(rc.question,rc.answerid,rc.user);
	}
