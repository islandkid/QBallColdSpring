component {
	...
	function startAuthenticate(any rc) {
		...
		if(arrayLen(rc.loginerrors)) {
			...
			rc.data = getUserService().authenticate(trim(rc.username),trim(rc.password));
		}
	}
	...
	function startRegister(any rc) {
		...
		if(arrayLen(rc.registererrors)) {
			...
			rc.registererrors = getUserService().register(trim(rc.username),trim(rc.password),rc.email);
		}
	}
